...
import 'package:goldcircle/pages/client/client_find_creators.dart';
...
class Creator {
  ...
  factory Creator.mock(String categoryId, RangeValues budgetRange) {
...
class Offer {
  ...
  factory Offer.mock(String categoryId, RangeValues budgetRange) {
    ...
    return Offer(
      ...
      isCreatorVerified: random.nextBool(),
      isCreatorOnline: random.nextDouble() > 0.3,
...
class CreatorsListPage extends StatefulWidget {
  ...
  @override
  State<CreatorsListPage> createState() => _CreatorsListPageState();
}

class _CreatorsListPageState extends State<CreatorsListPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final mockCreators = List.generate(
  ...
  List<Creator> get filteredAndSortedCreators {
    var filtered = _creators.where((provider) {
      // Apply filters
  ...
  List<Offer> get filteredAndSortedOffers {
    var filtered = _offers.where((offer) {
      // Apply filters
      if (_onlineOnly && !offer.isCreatorOnline) return false;
      if (_verifiedOnly && !offer.isCreatorVerified) return false;
      if (_priceFilter != null) {
          ...
          children: [
            Padding(
              ...
              child: Column(
                children: [
                  Expanded(
                    child: _selectedTabIndex == 0 ? _buildOffersTab() : _buildCreatorsTab(),
                  ),
                ],
              ),
            ),
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Icon(
              PhosphorIcons.tag(),
              color: isSelected ? AppStyles.textBlack : AppStyles.textSecondary ,
              size: 20.0,
            )
            :
            Icon(
  ...
  Widget _buildCreatorsContent() {
    ...
    if (filteredAndSortedCreators.isEmpty) {
      return _buildEmptyState();
    }

    return ListView.builder(
      padding: const EdgeInsets.only(top: 40, bottom: 20, left: 20, right: 20,),
      itemCount: filteredAndSortedCreators.length,
      itemBuilder: (context, index) {
        return _buildCreatorCard(filteredAndSortedCreators[index]);
      },
    );
  }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            _selectedTabIndex == 0
                ? 'Finding the best creators for you...'
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            Text(
              _selectedTabIndex == 0 ? 'Error loading creators' : 'Error loading offers',
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            Text(
              _selectedTabIndex == 0 ? 'No creators found' : 'No offers found',
              style: AppStyles.h3.copyWith(
                color: AppStyles.textPrimary,
              ),
            ),
  ...
  Widget _buildCreatorCard(Creator provider) {
    final isPinned = _pinnedCreators.contains(provider.id);
        ...
        child: Stack(
          children: [
            ...
            Positioned(
              ...
              right: 12,
              child: GestureDetector(
                onTap: () => _toggleCreatorPin(provider.id),
                child: Stack(
                  alignment: Alignment.center,
                  children: [
                    // Fill layer (underneath)
  ...
  void _onMessageCreator(Creator provider) {
  ...
  void _toggleCreatorPin(String providerId) {
    setState(() {
      if (_pinnedCreators.contains(providerId)) {
        _pinnedCreators.remove(providerId);
      } else {
        _pinnedCreators.add(providerId);
      }
    });
  }
  ...
  String _formatPrice(double price) {
    // if (price >= 1000) {
    //   return 'GHS${(price / 1000).toStringAsFixed(price % 1000 == 0 ? 0 : 1)}K';
    // }
    return 'GHS${price.toStringAsFixed(0)}';
  }
                ...
                children: [
                  ...
                  SwitchListTile(
                    ...
                    subtitle: Text(_selectedTabIndex == 0
                        ? 'Show only creators currently online'
                        : 'Show only offers from creators currently online'),
                    value: _onlineOnly,
                    onChanged: (value) {
                      setModalState(() {
                        _onlineOnly = value;
                  ...
                  SwitchListTile(
                    ...
                    subtitle: Text(_selectedTabIndex == 0
                        ...
                        : 'Show only offers from verified creators'),
                    value: _verifiedOnly,
                    onChanged: (value) {
                      setModalState(() {
                        _verifiedOnly = value;
