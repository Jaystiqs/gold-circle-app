...
class ServiceCategory {
  ...
  factory ServiceCategory.fromFirestore(DocumentSnapshot doc, {String? parentId}) {
    ...
    return ServiceCategory(
      ...
      parentId: parentId ?? data['parentId'], // Use passed parentId if provided, otherwise get from data
...
class FindCreatorsPage extends StatefulWidget {
  const FindCreatorsPage({super.key});

  @override
  State<FindCreatorsPage> createState() => _FindCreatorsPageState();
}

class _FindCreatorsPageState extends State<FindCreatorsPage>
  ...
  Future<void> _loadCategories() async {
    ...
    for (var parentDoc in parentSnapshot.docs) {
      // DON'T add parent category to the list
      // allCategories.add(ServiceCategory.fromFirestore(parentDoc));

      // Load subcategories for this parent
      final subcategoriesSnapshot = await parentDoc.reference
          .collection('subcategories')
          .where('isActive', isEqualTo: true)
          .orderBy('sortOrder')
          .get();

      // Add ONLY subcategories with parentId set to the parent's ID
      for (var subDoc in subcategoriesSnapshot.docs) {
        allCategories.add(ServiceCategory.fromFirestore(subDoc, parentId: parentDoc.id));
      }
    }
