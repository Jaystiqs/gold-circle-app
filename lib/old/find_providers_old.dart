...
// import 'package:goldcircle/client_creators_list.dart';
...
//     _loadRecentSearches(); // Load from Firestore
//   }
//
//   // NEW METHOD: Load recent searches from Firestore or local storage
...
//   }
//
//   // Load recent searches from Firestore (logged in users)
...
//   }
//
//   // Load recent searches from local storage (not logged in users)
...
//   }
//
//   // NEW METHOD: Add search query to recent searches
...
//   }
//
//   // Save recent search to Firestore (logged in users)
//   Future<void> _saveRecentSearchToFirestore(String query, User user) async {
//     try {
//       // Use query as document ID to avoid duplicates
...
//   }
//
//   // Save recent searches to local storage (not logged in users)
...
//   }
//
//   // Optional: Method to sync local searches to Firestore when user logs in
//   Future<void> _syncLocalSearchesToFirestore() async {
//     try {
//       final user = FirebaseAuth.instance.currentUser;
//       if (user == null) return;
//
//       // Load local searches
//       final prefs = await SharedPreferences.getInstance();
//       final localSearches = prefs.getStringList(_recentSearchesKey) ?? [];
//
//       if (localSearches.isEmpty) return;
//
//       // Save each local search to Firestore
//       for (String query in localSearches.reversed) { // Reverse to maintain order
//         await _saveRecentSearchToFirestore(query, user);
//       }
//
//       // Clear local searches after sync
//       await prefs.remove(_recentSearchesKey);
//
//       // Reload from Firestore to get the synced data
//       await _loadRecentSearchesFromFirestore(user);
//     } catch (e) {
//       print('Error syncing local searches to Firestore: $e');
//     }
//   }
//
//   // Optional: Method to clear all recent searches
...
//   }
//
//   // NEW METHOD: Execute a recent search
//   void _executeRecentSearch(String query) {
//     setState(() {
//       _searchQuery = query;
//     });
//     // Don't add to recent searches here since it's already in recent searches
...
//     final snapshot = await FirebaseFirestore.instance
//         .collection('categories') // Changed from 'service_categories' to 'categories'
...
//   }
//
//   // Add this list near the top of the class with other data
...
//     });
//     // Remove the animation for immediate response
...
//   }
//
//   // NEW METHOD: Build recent searches section
...
//       return const SizedBox.shrink(); // Don't show section if no recent searches
//     }
//
//     return Column(
...
//   }
//
//   // NEW METHOD: Build individual recent search item
...
//                       // Disable scrolling when timeline section is expanded to prevent conflicts
...
//                 ),
//               ),
//               // Search bar that triggers full screen
//               GestureDetector(
...
//               ),
//               const SizedBox(height: 20),
//               // Categories list with simplified scroll detection
//               Expanded(
...
//                         crossAxisAlignment: CrossAxisAlignment.start,
//                         children: [
//                           // MODIFIED: Show recent searches instead of "All categories"
//                           _buildRecentSearches(),
//                           // Show categories or loading placeholder
...
//                             )
//                           else
//                           // Show first 6 categories to encourage scrolling
...
//
//               const SizedBox(height: 20),
//               // Recent searches and all categories
//               Expanded(
//                 child: Padding(
//                   padding: const EdgeInsets.symmetric(horizontal: 20),
//                   child: Column(
//                     crossAxisAlignment: CrossAxisAlignment.start,
//                     children: [
//                       // MODIFIED: Show recent searches in full screen mode too
//                       if (_searchQuery.isEmpty && !_isLoadingRecentSearches && _recentSearches.isNotEmpty) ...[
//                         _buildRecentSearches(),
//                         const SizedBox(height: 8),
//                       ],
//                       Text(
//                         _isAiSearchActive && _aiRecommendedCategories.isNotEmpty
//                             ? 'All categories'
//                             : 'All categories',
//                         style: TextStyle(
//                           fontSize: 14,
...
//       child: InkWell(
//         onTap: () {
//           // Save current search query as recent search when category is selected
...
//       isCompleted: true, // Budget always has a value now
...
//                     const SizedBox(height: 16),
//
//                     // Min/Max labels positioned under the thumbs
//                     Row(
...
//                     const SizedBox(height: 12),
//
//                     // Value display cards positioned under the labels
//                     Row(
//                       mainAxisAlignment: MainAxisAlignment.spaceBetween,
//                       children: [
...
//                     const SizedBox(height: 30),
//
//                     // Common budget range pills - Horizontally Scrollable
//                     Text(
//                       'Suggested ranges',
//                       style: TextStyle(
//                           fontSize: 14,
...
//                         padding: const EdgeInsets.symmetric(horizontal: 4), // Small padding for first/last items
//                         child: Row(
...
//                                   decoration: BoxDecoration(
//                                     color: isSelected
//                                         ? AppStyles.blackMedium  // Changed from AppStyles.goldPrimary
//                                         : Colors.white,
...
//                                       BoxShadow(
//                                         color: AppStyles.textPrimary.withOpacity(0.25),  // Changed from AppStyles.goldPrimary
//                                         blurRadius: 8,
...
//                 ElevatedButton(
//                   onPressed: () {
//                     // Budget values are already set, move to next section
//                     _expandSection(2); // Open timeline section
//                   },
...
//           ),
//
//           // Toggle buttons for Days, Weeks, Months
//           Container(
//             margin: const EdgeInsets.symmetric(horizontal: 20),
...
//           const SizedBox(height: 20),
//
//           // Circular Timeline Selector for all categories
//           Expanded(
//             child: Container(
//               child: Column(
//                 children: [
...
//                     key: ValueKey(selectedTimelineCategory), // This forces widget replacement
...
//                 selectedTimelineCategory = 'Days'; // Reset to default
//                 daysValue = 7.0;
//                 weeksValue = 2.0;
//                 monthsValue = 3.0;
//                 _expandSection(0);
...
//                     selectedService: selectedCategory!, // Changed from selectedService to selectedCategory
